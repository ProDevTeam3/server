...
router.get("/industryByNationality", async (req, res) => {
  try {
    ...
    const nationalities = citizens.reduce((citizen, accumulator) => {
      ...
      const industriesDuplicates = citizen.contract.map(contractSingle => contractSingle.company.industry)
      const industries = industriesDuplicates.filter((v, i, a) => a.indexOf(v) === i);
      ...
      if(isNationalityPresent) {
        ...
        return {
          ...accumulator,
          [nationality]: industries.reduce((industry, accumulator2) => ({...accumulator2, [industry]: 1}), {})
        }
      }
    }, {})
